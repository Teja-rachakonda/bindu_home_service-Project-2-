      ...
      {leads.length === 0 ? (
        ...
        <div className="flex flex-col gap-3">
          {leads.map((l) => {
            ...
            // Normalise to an international number so wa.me opens the right chat.
            // A 10-digit local number is assumed Canada/US (+1).
            const digits = (l.phone || "").replace(/[^\d]/g, "");
            const waPhone = digits.length === 10 ? `1${digits}` : digits;
            const greeting = `Hi ${l.firstName || "there"}, thanks for your interest${
              l.dealName ? ` in ${l.dealName}` : ""
            }! How can I help you?`;
            return (
              <div key={l.id} className="rounded-xl border border-gray-200 p-3">
                ...
                {waPhone && (
                  <a
                    href={`https://wa.me/${waPhone}?text=${encodeURIComponent(greeting)}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="mt-2 flex w-full items-center justify-center gap-2 rounded-lg bg-whatsapp py-2 text-sm font-bold text-white active:scale-[0.99]"
                  >
